...
class _FrictionLossScreenState extends State<FrictionLossScreen> {
  ...
  void _calculateNhidrolik() {
    ...
    if (sicaklik == 0 || debi == 0 || cap == 0 || uzunluk == 0) {
      ...
      setState(() {
        errorMessage = '';
        dynamic calcResult = PumpEfficiency.calcColebrook(
          double.parse(sicaklikController.text),
          double.parse(debiController.text),
          double.parse(boruCapiController.text),
          double.parse(boruUzunluguController.text),
        );

        if (calcResult is double) {
          result = calcResult;
        } else {
          errorMessage = 'Hesaplama hatası: Geçersiz sonuç türü.';
          result = 0.0;
        }
      });
    }
            ...
            children: <Widget>[
              ...
              ElevatedButton(
                onPressed: () {
                  ...
                  var headLoss = (colebrook["fd"] * b_len * math.pow(u, 2)) /
                      (diameter * (2 * 9.7905));
              ...
              //if (result.isNotEmpty)
                Text(
                  'Sürtünme:  ${result.toStringAsFixed(2)} m',
                  style: TextStyle(
